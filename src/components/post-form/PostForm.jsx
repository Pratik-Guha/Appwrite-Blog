...
export default function PostForm({ post })  {
    ...
    const submit = async (data) => {
      if (post) {
          ...
          if (file) {
              // Check if post and post.featuredImage are define
                  service.deleteFile(post.faturedimage);
          }
  
          const dbPost = await service.updatePost(
              post.$id, // Check if post is defined
              {
                  ...data,
      ...
      <div className="w-1/3 px-2">
        ...
        {post && (
          <div className="w-full mb-4">
            <img
              src={service.getFilePreview(post.faturedimage)}
              alt={post.title}
        ...
        />
        <Button type="submit" bgColor={post ? "bg-green-500" : undefined} className="w-full hover:bg-blue-400">
          {post ? "Update" : "Submit"}
        </Button>
      </div>
    </form>
